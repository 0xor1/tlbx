    ...
    <div v-else>
      ...
      <div v-else>
        <div class="projects">
          <table>
            ...
            <tr
              class="row"
              @click="
                $u.rtr.goto(`/host/${p.host}/project/${p.id}/task/${p.id}`)
              "
              v-for="(p, index) in ps"
              :key="p.id"
            >
              ...
              <td
                v-if="isMe"
                class="action"
                @click.stop="toggleDeleteIndex(index)"
                title="delete safety"
              >
                <img src="@/assets/trash.svg" />
              </td>
              <td
                v-if="isMe && deleteIndex === index"
                class="action confirm-delete"
                @click.stop="trash(p, index)"
                title="delete"
              >
          ...
          <button class="load-more" v-if="psMore" @click="loadMore()">
            load more
          </button>
        </div>
        <div v-if="others.length > 0" class="others">
          ...
          <button class="load-more" v-if="othersMore" @click="loadMoreOthers()">
            load more
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    loadMoreOthers() {
      ...
      this.$api.project
        .get({ host: this.host, others: true, after })
        .then((res) => {
          for (let i = 0; i < res.set.length; i++) {
            this.others.push(res.set[i]);
          }
