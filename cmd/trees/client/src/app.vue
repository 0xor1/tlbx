<template>
  <div class="app-root">
    <div
      v-bind:class="{
        ...
        'show-project-activity':
          showProjectActivity && showProjectActivityToggle,
      }"
      class="body"
    >
    ...
    <div
      ...
      class="menu slide-out"
    >
      <div class="opts">
        <div class="btn" v-if="me" @click.stop.prevent="goHome()">home</div>
        <div class="btn" @click.stop.prevent="goToLatest()">latest</div>
        <div class="btn" @click.stop.prevent="showFields = !showFields">
          show
        </div>
        <div class="fields" v-if="showFields">
          <div
            v-for="(f, index) in availFields"
            :key="index"
            @click.stop.prevent="$root.show[f] = !$root.show[f]"
          >
            <span
              >{{ f
              }}<input @click.stop type="checkbox" v-model="$root.show[f]"
            /></span>
          </div>
        </div>
      ...
      <div title="report a bug" class="bug">
        <a
          href="https://github.com/0xor1/tlbx/issues/new?assignees=0xor1&labels=bug%2C+trees&template=trees-bug-report.md&title=%5Btrees%5D+%5Bbug%5D"
          target="_blank"
    ...
    <div
      ...
      class="slide-out-toggle menu-toggle"
    >
      <div></div>
      <div></div>
      <div></div>
    ...
    <div
      v-if="showProjectActivityToggle && showProjectActivity"
      v-bind:class="{ show: showProjectActivity }"
      class="project-activity slide-out"
    >
      <div
        class="exclude-deleted"
        @click.stop="toggleProjectActivityExcludeDeleted"
      >
        <input
          id="hide-deleted"
          :checked="projectActivityExcludeDeleted"
          type="checkbox"
        /><label for=""> hide deleted</label>
      </div>
      <div
        v-if="me != null"
        class="enable-realtime"
        @click.stop.prevent="toggleRealtime"
      >
        <input
          id="enable-realtime"
      ...
      <div class="entries">
        <div
          :class="{
            entry: true,
            'task-deleted': a.taskDeleted,
            deleted: a.itemDeleted,
          }"
          v-for="a in projectActivity"
          :key="a.occurredOn + a.item + a.user"
          @click.stop.prevent="gotoActivityTask(a)"
        >
          ...
          <span v-else-if="a.itemType == 'vitem'">
            ...
            <strong>{{
              a.extraInfo.type == "time"
                ? $u.fmt.time(a.extraInfo.inc)
                : $u.fmt.currencySymbol(project.currencyCode) +
                  $u.fmt.cost(a.extraInfo.inc, true)
            }}</strong>
          ...
          <span v-else-if="a.itemType == 'comment'">
            ...
            <strong :title="a.extraInfo">{{
              $u.fmt.ellipsis(a.extraInfo, 25)
            }}</strong>
            on <strong>{{ a.taskName }}</strong>
          </span>
          <br />
      ...
      <button
        v-if="!loadingProjectActivity && moreProjectActivity"
        @click.stop.prevent="loadMoreProjectActivity"
      >
        load more
      </button>
      <div v-if="loadingProjectActivity && projectActivity.length > 0">
        loading...
      </div>
    </div>
    <div
      title="project activity"
      v-if="showProjectActivityToggle"
      @click.stop.prevent="projectActivityToggle()"
      class="slide-out-toggle project-activity-toggle"
    >
  ...
  computed: {
    availFields() {
      return this.fields.filter((f) => {
        return (
          f != "file" || this.project == null || this.project.fileLimit > 0
        );
      });
    },
  },
  ...
  methods: {
    initState() {
      return {
        ...
        projectActivityExcludeDeleted:
          this.projectActivityExcludeDeleted === false ? false : true,
        realtimeEnabled:
          Notification.permission === "granted" && this.$api.fcm.isEnabled(),
        ...
        fields: ["date", "user", "time", "cost", "file", "task"],
        me: null,
      };
    },
    init() {
    ...
    toggleProjectActivityExcludeDeleted() {
      if (!this.loadingProjectActivity) {
        this.projectActivityExcludeDeleted = !this
          .projectActivityExcludeDeleted;
    ...
    loadMoreProjectActivity() {
      if (
        this.$u.rtr.project() != null &&
        this.showProjectActivity &&
        this.moreProjectActivity
      ) {
        ...
        this.$api.project
          .getActivities({
            ...
            occurredBefore: this.projectActivity[
              this.projectActivity.length - 1
            ].occurredOn,
          })
          .then((res) => {
        ...
        if (
          ...
          this.$api.project
            ...
            .then((res) => {
              this.loadingProjectActivity = false;
              if (
                (this.projectActivity.length == 0 && res.set.length == 0) ||
                ...
                if (
                  this.projectActivityCurrentPollDelayMs >
                  this.projectActivityMaxPollDelayMs
                ) {
