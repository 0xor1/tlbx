...
CREATE DATABASE trees_users CHARACTER SET = 'utf8mb4' COLLATE = 'utf8mb4_unicode_ci';
USE trees_users;
...
CREATE EVENT userRegistrationCleanup ON SCHEDULE EVERY 24 HOUR STARTS CURRENT_TIMESTAMP + INTERVAL 1 HOUR DO
DELETE FROM users
WHERE activatedOn = CAST('0000-00-00 00:00:00.000' AS DATETIME(3))
    AND registeredOn < DATE_SUB(NOW(), INTERVAL 7 DAY);
...
CREATE EVENT fcmTokenCleanup ON SCHEDULE EVERY 24 HOUR STARTS CURRENT_TIMESTAMP + INTERVAL 1 HOUR DO
DELETE FROM fcmTokens
WHERE createdOn < DATE_SUB(NOW(), INTERVAL 2 DAY);
